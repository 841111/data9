...
from .config import API_TIMEOUT_SECONDS, REPORT_DIR
...
def _process_single_face(face_data: tuple) -> GroupRecognitionItem | None:
    image, coords, students, activity_name = face_data
    x, y, w, h = coords
    try:
        ...
        insert_activity(activity_name, matched.student_id, matched.student_name)
        ...
        # Keep group flow robust: a single face failure should not fail the whole request.
        return None
...
def check_attendance(classroom_id: str, image_base64: str) -> AttendanceResult:
    try:
...
def recognize_group_photo(activity_name: str, raw: bytes) -> GroupRecognitionResponse:
    ...
    task_data = [(image, face_coords, students, activity_name) for face_coords in faces]
